...
const MyTimeLogs = () => {
  ...
  const { data: listResp, isLoading: isLogsLoading, error: logsError } = useListTimeLogsQuery({
    search: '',
    page,
    limit,
    ...
    userId: userInfo?._id || '', // Pass current user's ID - required for MyTimeLogs
    ...
    skip: !userInfo?._id || isUserLoading, // Skip the query if userInfo.id is not available or still loading
  });

  // Debug logging
  console.log('MyTimeLogs API call:', {
    userLoading: isUserLoading,
    userData: user,
    userInfo: userInfo,
    userId: userInfo?._id,
    logsLoading: isLogsLoading,
    hasLogsData: listResp,
    logsError: logsError,
    skip: !userInfo?._id || isUserLoading
  });
  ...
  if (isUserLoading || !userInfo?._id) {
    return (
      <div className="space-y-6">
        <div className="text-center py-12">
          <p className="text-lg text-muted-foreground">
            {isUserLoading ? 'Loading user information...' : 'User information not available'}
          </p>
        </div>
      </div>
    );
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2">
                            {columnOrder.map((key) => (
                              <div 
                                key={key} 
                                className={`flex items-center space-x-2 p-2 rounded-md cursor-move hover:bg-gray-50 transition-colors ${
                        ...
                        return flatLogs.map((log: any, idx: number) => {
                          ...
                          return (
                            <TableRow key={log.id} className={`border-b border-border hover:bg-muted/30 transition-colors h-12`}>
                              {viewMode === 'flat' ? (
                                <>
                                  <TableCell className={`p-4 ${isExpanded ? `${borderColor} ${isLastInGroup ? 'border-b-2 rounded-bl-md' : ''} border-l-2` : ''}`}><input type="checkbox" checked={selectedIds.has(log.id)} onChange={(e) => toggleSelect(log.id, e.target.checked)} /></TableCell>
                                  {columnOrder.map((key) => {
                                    if (!visibleColumns[key as keyof typeof visibleColumns]) return null;
