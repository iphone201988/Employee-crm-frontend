...
const ClientDetailsDialog = ({
  ...
  const [notes, setNotes] = useState([{
    ...
    id: '2',
    text: 'Discussed Q4 tax planning strategies. Client interested in pension contributions.',
    timestamp: '2024-02-20T14:15:00Z',
    user: 'Sarah Johnson'
  }, {
    id: '3',
    text: 'Updated contact information. New phone number provided.',
    timestamp: '2024-03-10T09:45:00Z',
    user: 'John Smith'
  }]);
  const [newNote, setNewNote] = useState('');
  const taxOptions = ['RCT', 'PAYE-EMP', 'VAT', 'IT', 'CGT', 'CT', 'DWT', 'ELEVY'];
  const handleAddNote = () => {
    if (newNote.trim()) {
      const newNoteObj = {
        ...
        user: 'Current User' // In a real app, this would come from the auth context
      };
  ...
  const handleSave = () => {
    ...
    console.log('Saving client data:', editableData);
    setIsEditing(false);
  };
  const handleCancel = () => {
      ...
      <DialogContent className="max-w-6xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center justify-between">
            <DialogTitle className="text-xl font-bold">{clientData.name}</DialogTitle>
        ...
        <Tabs defaultValue="details" className="w-full">
          <TabsList className="grid w-full grid-cols-8">
            ...
            <TabsTrigger value="notes">Notes (3)</TabsTrigger>
              ...
              <CardContent className="grid gap-4 pt-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    {isEditing ? <Input value={editableData.clientRef} onChange={e => setEditableData(prev => ({
                    ...prev,
                    clientRef: e.target.value
                  }))} /> : <div className="p-2 bg-muted rounded">{editableData.clientRef}</div>}
                  </div>
                  <div className="grid gap-2">
                    <Label>Client Name</Label>
                    {isEditing ? <Input value={editableData.name} onChange={e => setEditableData(prev => ({
                    ...prev,
                    name: e.target.value
                  }))} /> : <div className="p-2 bg-muted rounded">{editableData.name}</div>}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    {isEditing ? <Input value={editableData.customerNumber || ''} onChange={e => setEditableData(prev => ({
                    ...prev,
                    customerNumber: e.target.value
                  }))} /> : <div className="p-2 bg-muted rounded">{editableData.customerNumber || 'Not specified'}</div>}
                  </div>
                  <div className="grid gap-2">
                    <Label>Client Type</Label>
                    {isEditing ? <Input value={editableData.clientType || ''} onChange={e => setEditableData(prev => ({
                    ...prev,
                    clientType: e.target.value
                  }))} /> : <div className="p-2 bg-muted rounded">{editableData.clientType || 'Not specified'}</div>}
                  </div>
                </div>
                
                <div className="grid gap-2">
                  <Label>Address</Label>
                  {isEditing ? <Textarea value={editableData.address || ''} onChange={e => setEditableData(prev => ({
                  ...prev,
                  address: e.target.value
                }))} /> : <div className="p-2 bg-muted rounded">{editableData.address || 'Not specified'}</div>}
                </div>
                
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label>Contact Person</Label>
                    {isEditing ? <Input value={editableData.contactPerson || ''} onChange={e => setEditableData(prev => ({
                    ...prev,
                    contactPerson: e.target.value
                  }))} /> : <div className="p-2 bg-muted rounded">{editableData.contactPerson || 'Not specified'}</div>}
                  </div>
                  <div className="grid gap-2">
                    <Label>Email</Label>
                    {isEditing ? <Input type="email" value={editableData.email || ''} onChange={e => setEditableData(prev => ({
                    ...prev,
                    email: e.target.value
                  }))} /> : <div className="p-2 bg-muted rounded">{editableData.email || 'Not specified'}</div>}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label>Phone</Label>
                    {isEditing ? <Input value={editableData.phone || ''} onChange={e => setEditableData(prev => ({
                    ...prev,
                    phone: e.target.value
                  }))} /> : <div className="p-2 bg-muted rounded">{editableData.phone || 'Not specified'}</div>}
                  </div>
                  <div className="grid gap-2">
                    <Label>Take On Date</Label>
                    {isEditing ? <Input type="date" value={editableData.takeOnDate || ''} onChange={e => setEditableData(prev => ({
                    ...prev,
                    takeOnDate: e.target.value
                  }))} /> : <div className="p-2 bg-muted rounded">{editableData.takeOnDate || 'Not specified'}</div>}
                  </div>
                </div>
                ...
                <div className="grid gap-2">
                  ...
                  {isEditing ? <div className="space-y-2">
                      <div className="flex flex-wrap gap-2">
                        {editableData.clientTags && editableData.clientTags.map((tag, index) => <Badge key={index} variant="secondary" className="cursor-pointer">
                            {tag}
                            <X className="h-3 w-3 ml-1" onClick={() => {
                        const newTags = editableData.clientTags?.filter((_, i) => i !== index) || [];
                      ...
                      <div className="flex gap-2">
                        <Input placeholder="Add new tag..." onKeyDown={e => {
                      if (e.key === 'Enter') {
                        const input = e.target as HTMLInputElement;
                        const newTag = input.value.trim();
                        if (newTag && !editableData.clientTags?.includes(newTag)) {
                          setEditableData(prev => ({
                            ...prev,
                            clientTags: [...(prev.clientTags || []), newTag]
                          }));
                          input.value = '';
                        }
                      }
                      ...
                      {editableData.clientTags && editableData.clientTags.length > 0 ? editableData.clientTags.map((tag, index) => <Badge key={index} variant="secondary">{tag}</Badge>) : <span className="text-muted-foreground">No tags assigned</span>}
                    </div>}
                </div>
