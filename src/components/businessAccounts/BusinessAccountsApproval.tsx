...
import { useGetAllCompanyMembersQuery, useUpdateTeamMembersMutation } from '@/store/teamApi';
...
const BusinessAccountsApproval: React.FC<ApprovalsContentProps> = ({ onUnsavedChangesChange }) => {
  ...
  const { data: teamData, isLoading, error } = useGetAllCompanyMembersQuery({ page, limit });
  const [updateTeamMembers] = useUpdateTeamMembersMutation();

  const pagination = teamData?.data?.pagination;

  useEffect(() => {
    if (teamData?.data?.companyMembers) {
      const transformedMembers = teamData.data.companyMembers.map(transformToApprovals);
  ...
  const handlePermissionChange = (memberId: string, permissionKey: keyof ApprovalsTeamMember['permissions'], checked: boolean) => {
    const originalMember = teamData?.data?.companyMembers?.find(m => m._id === memberId);
...
export default BusinessAccountsApproval;