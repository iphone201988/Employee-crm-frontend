...
const BusinessAccountAccess: React.FC<AccessContentProps> = ({ onUnsavedChangesChange }) => {
  ...
  useEffect(() => {
    if (teamData?.data?.companyMembers) {
      const transformedMembers = teamData?.data?.companyMembers.map(transformToAccess);
...
export default BusinessAccountAccess;